        ...
        <div className="profile">
          ...
          <div className="social">
            <Tooltip title="impfans">
              <a href="https://github.com/impfans" target="_blank">
                <GithubOutlined />
              </a>
            </Tooltip>
            ...
            <Tooltip title="杜跃飞">
              <a href="https://twitter.com/yuefeidu2021" target="_blank">
                <TwitterOutlined />
              </a>
            </Tooltip>
          </div>
          ...
          {newBlog.map((item: any, index: number) => (
            ...
            >
              <Row className="item" gutter={8}>
                <Col span={8}>
                  <img src={`${AXIOS_BASE}/${item.img}`} alt="" />
                </Col>
                <Col span={14}>
                  <h4>{item.title}</h4>
                  <i>{moment(item.created_at).format('YYYY-MM-DD HH:mm')}</i>
                </Col>
              </Row>
            </Link>
          ))}
        ...
        <div className="card" id="category">
          ...
          {category.map((item: any, index: number) => (
            <Row
              ...
              gutter={8}
              onClick={() => {
                Router.push(`/?cid=${item.id}#article-list`)
              }}
            >
          ...
          <div className="tags">
            {tags.map((item, index) => (
              <span
                key={index}
                style={{
                  color: `rgb(${Math.floor(Math.random() * 201)}, ${Math.floor(
                    Math.random() * 201
